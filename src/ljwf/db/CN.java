...
//			p.setProperty("url","jdbc:mysql://" + dbip + ":3306/" + dbname + "?useUnicode=true&characterEncoding=gbk&zeroDateTimeBehavior=convertToNull");
			p.setProperty("url", "jdbc:oracle:thin:@"+dbip+":1521:"+dbname);
...
//			p.setProperty("logAbandoned", logAbandoned);
	...
	public synchronized static CN getCn() throws Exception {
		// get a new cn
		...
		if (false) {
			...
			DataSource ds = (DataSource) ctx.lookup("jdbc/mportal");
			cn = ds.getConnection();
		}
		if (false) {
	...
	public CN(java.sql.Connection cn) {
		db_cn = cn;
	}
	...
	static CN InsertCn(java.sql.Connection cn) {
		return new CN(cn);
	}
